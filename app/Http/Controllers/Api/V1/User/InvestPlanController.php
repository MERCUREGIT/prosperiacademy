...
class InvestPlanController extends Controller
{
    ...
    public function planPurchase(Request $request, InvestmentPlan $invest_plan) {
        ...
        if($validated['invest_amount'] < $invest_plan->min_invest_requirement || $validated['invest_amount'] > $invest_plan->max_invest) {
            return Response::error(['Your can invest minimum ' . get_amount($invest_plan->min_invest_requirement,$user_wallet->currency->code,4) . " to maximum " . get_amount($invest_plan->max_invest,$user_wallet->currency->code,4)],[],400);
        }
    ...
    public function profitLog() {
        $profit_logs = InvestmentProfitLog::auth()->orderByDesc("id")->select('id','user_id','invest_id','profit_amount','created_at')->with(['invest' => function($q) {
