                ...
                <div class="card-text">
                    <div class="d-sm-flex d-sm-block justify-content-between">
                        <div class="d-flex">
                            ...
                            <div class="">
                                ...
                                @if ($transaction->user_id == auth()->user()->id)
                                    <span class="badge badge-success">{{ __("Send") }}</span>
                                @elseif ($transaction->receiver_id == auth()->user()->id)
                                    <span class="badge badge-warning">{{ __("Received") }}</span>
                                @endif
                            </div>
                        </div>
                        ...
                        <div class="my-auto text-end pt-sm-0 pt-3 ps-sm-0 ps-5">
                            ...
                            @if ($transaction->user_id == auth()->user()->id)
                                <br>
                                <span class="text-white fs-5 pb-2">{{ get_amount($transaction->request_amount,$transaction->payment_currency,4)  }}</span>
                            @endif
                            <br>
                            <span class="text-white">{{ get_amount($transaction->receive_amount,$transaction->request_currency,4) }}</span>
                        </div>
                    </div>
                </div>
