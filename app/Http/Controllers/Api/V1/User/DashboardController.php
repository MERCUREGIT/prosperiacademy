...
class DashboardController extends Controller
{
    public function dashboard() {
        ...
        })->select('id','user_id','currency_id','balance','profit_balance','status','created_at')->with(['currency' => function($q) {
            $q->select('id','code');
        }])->get();
        ...
        $dashboard_cards = [
            'c_balance' => [
                ...
                'amount'        => $user_wallets?->first()?->balance ?? 0.00,
                'currency'      => $user_wallets?->first()?->currency?->code ?? "",
            ],
            
            'p_balance' => [
                'title'         => __("Profit Balance"),
                'amount'        => $user_wallets?->first()?->profit_balance ?? 0.00,
                'currency'      => $user_wallets?->first()?->currency?->code ?? "",
            ],
            
            'i_balance' => [
                'title'         => __("Investment Balance"),
                'amount'        => get_amount(UserHasInvestPlan::auth()->pluck('invest_amount')->sum(),null,8),
                'currency'      => $user_wallets?->first()?->currency?->code ?? "",
            ],
