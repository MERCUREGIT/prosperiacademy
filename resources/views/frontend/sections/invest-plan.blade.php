                        ...
                        <div class="pricing-card text-center">
                            ...
                            <div class="amount">
                                @if ($item->min_invest_offer > 0)
                                    ...
                                    <p>
                                        <span>{{ __("from") }} /</span>
                                        <del>{{ $default_currency->symbol . get_amount($item->min_invest,null,4) }}</del>
                                        <span>{{ $default_currency->symbol . get_amount($item->min_invest_offer,null,4) }}</span>
                                    </p>
                                @else
                                    <p><span>{{ __("from") }} /</span>{{ $default_currency->symbol . get_amount($item->min_invest,null,4) }}</p>
                                @endif
                            </div>
                            <div class="details">
                                ...
                                <p><i class="la la-check"></i>{{ __("Maximum Investment") }} {{ $default_currency->symbol . get_amount($item->max_invest,null,4) }}</p>
                                <p><i class="la la-check"></i> {{ __("Fixed Profit") }} {{ $default_currency->symbol . get_amount($item->profit_fixed,null,4) }}</p>
                                <p><i class="la la-check"></i> {{ __("Percentage Profit") }} {{ get_amount($item->profit_percent,"%",4) }}</p>
