@if ($basic_settings->kyc_verification == true && isset($kyc_data) && $kyc_data != null && $kyc_data->fields != null)
    ...
    <div class="custom-card mt-30">
        <div class="dashboard-header-wrapper">
            <h4 class="title text-white">{{ __("KYC Information") }} &nbsp; <span class="{{ auth()->user()->kycStringStatus->class }}">{{ auth()->user()->kycStringStatus->value }}</span></h4>
        </div>
        <div class="card-body">
                    ...
                    @foreach (auth()->user()->kyc->data ?? [] as $item)
                        <li>
                            @if ($item->type == "file")
                                ...
                                @endphp
                                <span class="kyc-title text-white">{{ $item->label }}:</span> 
